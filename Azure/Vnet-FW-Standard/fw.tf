...
resource "azurerm_firewall_policy" "standard_sku_fw_policies" {
...
locals {
  ...
  standard_sku_fw_policies = keys(azurerm_firewall_policy.standard_sku_fw_policies)
}

# Firewall

resource "azurerm_firewall" "standard_sku_FWs" {
  ...
  firewall_policy_id = [for policy in local.standard_sku_fw_policies : azurerm_firewall_policy.standard_sku_fw_policies[policy].id if azurerm_firewall_policy.standard_sku_fw_policies[policy].name == each.value.firewall_policy.name][0]
}
